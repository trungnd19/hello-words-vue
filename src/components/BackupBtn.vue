<template>
  <div @click="navigateToBackup" title="Backup & Restore" class="cursor-pointer">
    <svg class="icon-btn" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="1.1em" height="1.1em">
      <path
        fill="currentColor"
        d="M19.35 10.04A7.49 7.49 0 0 0 12 4C9.11 4 6.6 5.64 5.35 8.04A5.994 5.994 0 0 0 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z"
      />
    </svg>
  </div>
</template>

<script setup lang="ts">
import { useRoutePage } from "../composables/useRoutePage";

const { navigateToBackup } = useRoutePage();
</script>
